...
export const eliminarBodega = async (req, res) => {
  ...
  try {
    ...
    const bodegaEliminado = await Bodega.destroy({ where: { id_bodega: id } });
...
export const editarBodega = async (req, res) => {
  ...
  const { nombre_bodega, direccion_bodega, capacidad_bodega, precio_bodega, id_tipo_bodega, id_responsable_bodega } = req.body; // Obtener los datos actualizados del producto desde el cuerpo de la solicitud
  try {
    // Buscar el producto por su ID y actualizar sus datos
    const bodegaActualizado = await Bodega.update(
      { nombre_bodega, direccion_bodega, capacidad_bodega, precio_bodega, id_tipo_bodega, id_responsable_bodega },
      { where: { id_bodega: id } }
    );
